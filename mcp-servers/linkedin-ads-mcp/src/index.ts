...
async function handleGetCampaignPerformance(args: GetPerformanceArgs) {
  console.error('[LinkedInAds] get_campaign_performance called');
...
async function handleUpdateCampaignBudget(args: UpdateBudgetArgs) {
  console.error(`[LinkedInAds] update_campaign_budget called for ${args.campaign_id}`);
...
async function handlePauseCampaign(args: CampaignActionArgs) {
  console.error(`[LinkedInAds] pause_campaign called for ${args.campaign_id}`);
...
async function handleResumeCampaign(args: CampaignActionArgs) {
  console.error(`[LinkedInAds] resume_campaign called for ${args.campaign_id}`);
    ...
    tools: [
      {
        ...
        description: 'Get LinkedIn Ads campaign performance metrics for a date range.',
        inputSchema: {
          type: 'object',
          properties: {
            date_range: {
      ...
      },
      {
        ...
        description: 'Update the daily budget for a LinkedIn Ads campaign in USD.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      {
        name: 'pause_campaign',
        description: 'Pause an active LinkedIn Ads campaign',
        inputSchema: {
          type: 'object',
      ...
      },
      {
        ...
        description: 'Resume a paused LinkedIn Ads campaign',
        inputSchema: {
          type: 'object',
          properties: { campaign_id: { type: 'string' } },
          required: ['campaign_id'],
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    console.error(`[LinkedInAds] Error in ${name}:`, error);
    return formatToolResponse(
...
async function main() {
  try {
    ...
    console.error('[LinkedInAds] Client initialization failed, running in mock mode');
  }

  if (hasValidCredentials()) {
    console.error('[LinkedInAds] Running with valid credentials');
  } else {
    console.error('[LinkedInAds] Running in MOCK MODE');
  }
...
main().catch((error) => {
  console.error('[LinkedInAds] Fatal error:', error);
  process.exit(1);
});
