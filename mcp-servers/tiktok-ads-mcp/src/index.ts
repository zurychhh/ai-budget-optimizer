...
async function handleGetCampaignPerformance(args: GetPerformanceArgs) {
  console.error('[TikTokAds] get_campaign_performance called');
...
async function handleUpdateCampaignBudget(args: UpdateBudgetArgs) {
  console.error(`[TikTokAds] update_campaign_budget called for ${args.campaign_id}`);

  if (!args.new_budget || args.new_budget <= 0) {
    return formatToolResponse({ error: true, message: 'new_budget must be positive' }, true);
  }

  const campaign = await getCampaignById(args.campaign_id);
  if (!campaign) {
    return formatToolResponse({ error: true, message: `Campaign ${args.campaign_id} not found` }, true);
  }
...
async function handlePauseCampaign(args: CampaignActionArgs) {
  console.error(`[TikTokAds] pause_campaign called for ${args.campaign_id}`);
  await updateCampaignStatus(args.campaign_id, 'DISABLE');
...
async function handleResumeCampaign(args: CampaignActionArgs) {
  console.error(`[TikTokAds] resume_campaign called for ${args.campaign_id}`);
  await updateCampaignStatus(args.campaign_id, 'ENABLE');
    ...
    tools: [
      {
        ...
        description: 'Get TikTok Ads campaign performance metrics for a date range.',
        inputSchema: {
          type: 'object',
          properties: {
            date_range: {
      ...
      },
      {
        ...
        description: 'Update the daily budget for a TikTok Ads campaign in USD.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      {
        name: 'pause_campaign',
        description: 'Pause an active TikTok Ads campaign',
        inputSchema: {
          type: 'object',
      ...
      },
      {
        ...
        description: 'Resume a paused TikTok Ads campaign',
        inputSchema: {
          type: 'object',
          properties: { campaign_id: { type: 'string' } },
          required: ['campaign_id'],
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    console.error(`[TikTokAds] Error in ${name}:`, error);
    return formatToolResponse(
...
async function main() {
  try {
    ...
    console.error('[TikTokAds] Client initialization failed, running in mock mode');
  }

  if (hasValidCredentials()) {
    console.error('[TikTokAds] Running with valid credentials');
  } else {
    console.error('[TikTokAds] Running in MOCK MODE');
  }
...
main().catch((error) => {
  console.error('[TikTokAds] Fatal error:', error);
  process.exit(1);
});
